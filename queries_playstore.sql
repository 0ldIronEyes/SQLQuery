...
SELECT name,price,rating FROM analytics ORDER BY price DESC WHERE rating < 3 LIMIT 1;

--Find all apps with a min install not exceeding 50, that have a rating. Order your result by the highest rated first.
SELECT * FROM analytics WHERE min_installs <= 50 AND rating IS NOT NULL ORDER BY rating DESC;

--Find the names of all apps that are rated less than 3 with at least 1000 reviews.
...
--Find the most out of date app. Hint: You don't need to do it thi way, but it's possible to do with a subquery
SELECT * FROM analytics WHERE last_updated LIMIT 1;
...
--Find the app that has the highest proportion of min_installs to reviews, among apps that have been installed at least 100,000 times. Display the name 
